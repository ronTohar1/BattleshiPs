...
def main():
    ...
    # arguments = ["-e", "-l", "-v", "-a", "-lr", "-g", "-na", "-af", "-p", "-fd", "-mt"]
    
    sucssesful_runs =[]
    sucssesful_runs.append([3_000_000, 0.00001, 0.99, 'dqn', '[]', 'relu', 'CnnPolicy', 256, 'cnn'])
    sucssesful_runs.append([3_000_000, 0.00001, 0.99, 'dqn', '[]', 'relu', 'CnnPolicy', 512, 'cnn'])
    sucssesful_runs.append([3_000_000, 0.00001, 0.99, 'dqn', '[256]', 'relu', 'CnnPolicy', 256, 'cnn'])
    sucssesful_runs.append([3_000_000, 0.00001, 0.99, 'dqn', '[]', 'relu', 'CnnPolicy', 350, 'cnn'])
    sucssesful_runs.append([3_000_000, 0.000005, 0.99, 'dqn', '[]', 'relu', 'CnnPolicy', 512, 'cnn'])

    using_bp = not args.no_bp_strats # if the flag is set, we are not using BP strategies
    strats_flag = "-bp" if using_bp else "-nobp"
    counter = 0
    for ep, lr, gamma, agent, arch, activation, policy, feature_dim, model_type in sucssesful_runs:
        counter +=1
        ...
        print(f"({counter} srun)",f"Ran with args: ep-{ep}, log_path-{log_path}, verbose-{verbose}, agent-{agent}, lr-{lr}, gamma-{gamma}, bp-{using_bp}, architecture-{arch}, activation-{activation}, policy-{policy}, feature_dim-{str(feature_dim)}, model_type-{model_type}, and got return code: ", r.returncode)
