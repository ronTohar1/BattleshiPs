...
def main():
    ...
    for ep, lr, gamma, agent in product(episodes, lrs, gammas, agents):
        ...
        print(f"({counter})",f"Ran with args: ep-{ep}, log_path-{log_path}, verbose-{verbose}, agent-{agent}, lr-{lr}, gamma-{gamma}, bp-{args.bp_strats}(type:{type(args.bp_strats)}), and got return code: ", r.returncode)
